...
async def get_dashboard_stats(db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
    ...
    # Count OSC (only published)
    osc_result = await db.execute(select(func.count(Osc.id)).where(Osc.statut_publication == "publie"))
...
async def get_osc_by_region(
    ...
    query = (
        ...
        .outerjoin(Osc, (Osc.crasc_id == Crasc.id) & (Osc.statut_publication == "publie"))
...
async def get_osc_by_type(
    ...
    query = (
        ...
        .outerjoin(Osc, (Osc.type_id == OscType.id) & (Osc.statut_publication == "publie"))
